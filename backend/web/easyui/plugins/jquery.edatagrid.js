...
(function($){
			...
			onAfterEdit: function(index, row){
				...
				if (url){
					$.post(url, row, function(data){
						//data.isNewRecord = null;//原本打开时，新增失败后，编辑再提交会判断为更新操作而不是新增
				...
				$.messager.confirm(opts.destroyMsg.confirm.title,opts.destroyMsg.confirm.msg,function(r){
					if (r){
						...
						if (row.isNewRecord){
							...
							if (opts.destroyUrl){
								var idValue = row[opts.idField||'id'];
								$.post(opts.destroyUrl, {id:idValue,_csrf:$('meta[name="csrf-token"]').attr("content")}, function(result){
									if (opts.tree){
										dg.datagrid('reload');
										var t = $(opts.tree);
										var node = t.tree('find', idValue);
	...
	$.fn.datagrid.defaults.pageSize	= 20;//设置每页默认显示数量
